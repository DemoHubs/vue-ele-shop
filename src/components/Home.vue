      ...
      <el-aside :width="toggleMenuData?'65px':'200px'">
         <div class="toggle-menu" @click="toggleMenu">|||</div>
          <el-menu
                ...
                text-color="#fff"
                :collapse="toggleMenuData"
                :default-active="defActive"
                router
                active-text-color="#ffd04b">
                <el-submenu :index="menuIt.name" v-for="menuIt in menuData" :key="menuIt.name">
                  <template slot="title">
                    <i class="el-icon-location"></i>
                    <span>{{menuIt.meta.title}}</span>
                  </template>
                  <el-menu-item-group>
                    <template slot="title" > </template>
                    <el-menu-item @click="menuClick('/'+childMenuIt.component)" :key="childMenuIt.User" :index="'/'+childMenuIt.component"  v-for="childMenuIt in menuIt.children">{{childMenuIt.meta.title}}</el-menu-item>
...
<script>
var def_path_key="defActPath";
export default {
  data(){
    return {
      menuData: [],
      toggleMenuData: false,
      defActive:''
    }
  },
  created() {
      ...
      this.defActive=sessionStorage.getItem(def_path_key);
  },
  methods: {
    logout() {
